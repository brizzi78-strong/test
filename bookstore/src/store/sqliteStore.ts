...
import type { Book, Listing, Order, Quote, Seller } from '../domain/types.ts';
import type { Collection, Store } from './store.ts';

const TABLES = ['sellers', 'books', 'listings', 'orders', 'quotes'] as const;
...
export function createSqliteStore(path: string): SqliteStore {
  ...
  return {
    sellers: new SqliteCollection<Seller>(db, 'sellers'),
    books: new SqliteCollection<Book>(db, 'books'),
    listings: new SqliteCollection<Listing>(db, 'listings'),
    orders: new SqliteCollection<Order>(db, 'orders'),
    quotes: new SqliteCollection<Quote>(db, 'quotes'),
    close: () => db.close(),
  };
}
