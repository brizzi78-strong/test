 ...
 *   POST   /issues                GET /issues[?status=green|yellow|red]
 *   PUT    /issues/:id
 *   POST   /recognitions          GET /recognitions
...
import type { RoundingService } from '../service/roundingService.ts';
...
export function createRequestListener(
  ...
  route('POST', '/units', (ctx) => created(service.createUnit(ctx.userId, ctx.body)));
  route('GET', '/units', (ctx) => ok({ units: service.listUnits(ctx.userId) }));

  route('GET', '/templates', (ctx) => ok({ templates: service.listTemplates(ctx.userId) }));
  route('POST', '/templates', (ctx) => created(service.createTemplate(ctx.userId, ctx.body)));

  route('POST', '/rounds', (ctx) => created(service.createRound(ctx.userId, ctx.body)));
  route('GET', '/rounds', (ctx) => ok({ rounds: service.listRounds(ctx.userId) }));
  route('GET', '/rounds/:id', (ctx) => ok(service.getRound(ctx.userId, ctx.params.id!)));

  route('POST', '/issues', (ctx) => created(service.createIssue(ctx.userId, ctx.body)));
  route('GET', '/issues', (ctx) =>
    ok({ issues: service.listIssues(ctx.userId, ctx.query.get('status') ?? undefined) }),
  );
  route('PUT', '/issues/:id', (ctx) => ok(service.updateIssue(ctx.userId, ctx.params.id!, ctx.body)));

  route('POST', '/recognitions', (ctx) => created(service.createRecognition(ctx.userId, ctx.body)));
  route('GET', '/recognitions', (ctx) => ok({ recognitions: service.listRecognitions(ctx.userId) }));

  route('GET', '/summary', (ctx) => ok(service.summary(ctx.userId)));
  ...
  return (req, res) => {
    ...
    if (opts.gate && url.pathname !== '/health' && !authorized(req, opts.gate)) {
      res.writeHead(401, {
        'www-authenticate': `Basic realm="${opts.brandName ?? 'Cardinal Rounds'}"`,
    ...
    if (req.method === 'GET' && (url.pathname === '/' || url.pathname === '/index.html')) {
      try {
        ...
        if (opts.brandName) {
          html = html
            .replace('<title>Cardinal Rounds', `<title>${opts.brandName}`)
            .replace('<h1>Cardinal Rounds<', `<h1>${opts.brandName}<`);
        }
