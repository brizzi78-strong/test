/**
 * HTTP server wiring: assemble a RoundingService with a store and expose it
...
export interface AppOptions {
  /** Basic-auth gate; both must be set to enable it (ROUNDS_USER/ROUNDS_PASSWORD). */
  user?: string;
  password?: string;
  /** Site display name (BRAND_NAME), e.g. "Mercy West Rounding". */
  brandName?: string;
}

/**
 * Select a store from the environment: `ROUNDS_DB=/path/to/data.db` uses the
...
export function storeFromEnv(env: NodeJS.ProcessEnv = process.env): Store {
  return env.ROUNDS_DB ? createSqliteStore(env.ROUNDS_DB) : createInMemoryStore();
}
...
export function createApp(store: Store = storeFromEnv(), opts: AppOptions = {}): AppServer {
  const user = opts.user ?? process.env.ROUNDS_USER;
  const password = opts.password ?? process.env.ROUNDS_PASSWORD;
  ...
  const service = new RoundingService({ store });
