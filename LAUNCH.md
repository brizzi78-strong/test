...
| Uniswap pool | 200M HOP (80%) paired with 2–5 ETH |
| Treasury | 50M HOP (20%) in a publicly announced wallet |
...
| **Full launch dress rehearsal** — real Uniswap V2 stack deployed locally; deploy → seed 10M HOP/100 ETH pool → buyer swap (price verified incl. 0.3% fee) → renounceOwnership, all green | `scripts/rehearse-launch.ts` |
...
Engage a crypto-literate lawyer on: what HOP is for, how it's distributed,
...
```bash
HOP_NETWORK=mainnet HOP_TOKEN_ADDRESS=0x... HOP_AMOUNT=... ETH_AMOUNT=... \
  npx hardhat run scripts/add-liquidity.ts
```

- [ ] The HOP/ETH ratio you pass sets the launch price — sanity-check it.
