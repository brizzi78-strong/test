...
| Uniswap pool | 200M HERE (80%) paired with 2–5 ETH |
| Treasury | 50M HERE (20%) in a publicly announced wallet |
...
| Step | Where |
| --- | --- |
| Token contract (250M fixed supply — no mint, no burn; OpenZeppelin ERC20 + Ownable) | `contracts/Presence.sol` |
| Test suite — 17 tests (Foundry-style Solidity incl. fuzz + invariants, plus node:test/viem), all passing | `contracts/*.t.sol`, `test/Presence.ts` |
...
| Local deployment rehearsal (Ignition) | `ignition/modules/Presence.ts` |
...
| **Full launch dress rehearsal** — real Uniswap V2 stack deployed locally; deploy → seed 10M HERE/100 ETH pool → buyer swap (price verified incl. 0.3% fee) → renounceOwnership, all green | `scripts/rehearse-launch.ts` |
...
Engage a crypto-literate lawyer on: what HERE is for, how it's distributed,
...
npx hardhat ignition deploy ignition/modules/Presence.ts --network mainnet
...
- [ ] The HERE/ETH ratio you pass sets the launch price — sanity-check it.
