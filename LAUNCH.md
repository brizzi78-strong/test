...
| Uniswap pool | 200M CP17 (80%) paired with 2–5 ETH |
| Treasury | 50M CP17 (20%) in a publicly announced wallet |
...
| Step | Where |
| --- | --- |
| Token contract (250M fixed supply — no mint, no burn; OpenZeppelin ERC20 + Ownable) | `contracts/CP17.sol` |
| Test suite — 17 tests (Foundry-style Solidity incl. fuzz + invariants, plus node:test/viem), all passing | `contracts/*.t.sol`, `test/CP17.ts` |
...
| Local deployment rehearsal (Ignition) | `ignition/modules/CP17.ts` |
...
| **Full launch dress rehearsal** — real Uniswap V2 stack deployed locally; deploy → seed 10M CP17/100 ETH pool → buyer swap (price verified incl. 0.3% fee) → renounceOwnership, all green | `scripts/rehearse-launch.ts` |
...
Engage a crypto-literate lawyer on: what CP17 is for, how it's distributed,
...
npx hardhat ignition deploy ignition/modules/CP17.ts --network mainnet
...
```bash
CP17_NETWORK=mainnet CP17_TOKEN_ADDRESS=0x... CP17_AMOUNT=... ETH_AMOUNT=... \
  npx hardhat run scripts/add-liquidity.ts
```

- [ ] The CP17/ETH ratio you pass sets the launch price — sanity-check it.
