...
| Step | Where |
| --- | --- |
| Token contract (250M fixed supply — no mint, no burn; OpenZeppelin ERC20 + Ownable) | `contracts/Card.sol` |
| Test suite — 17 tests (Foundry-style Solidity incl. fuzz + invariants, plus node:test/viem), all passing | `contracts/*.t.sol`, `test/Card.ts` |
...
| Local deployment rehearsal (Ignition) | `ignition/modules/Card.ts` |
...
npx hardhat ignition deploy ignition/modules/Card.ts --network mainnet
