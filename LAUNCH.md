...
| Uniswap pool | 200M HOPE (80%) paired with 2–5 ETH |
| Treasury | 50M HOPE (20%) in a publicly announced wallet |
...
| Before mainnet | Publish the coin site at **cp17.org** (already live from `cp17-site/`) and fill in the story section. The coin and the book are kept deliberately separate: nothing about HOPE lives on thecardinalspromise.com, and cp17.org does not link to the book |
...
| Step | Where |
| --- | --- |
| Token contract (250M fixed supply — no mint, no burn; OpenZeppelin ERC20 + Ownable) | `contracts/HopeCoin.sol` |
| Test suite — 17 tests (Foundry-style Solidity incl. fuzz + invariants, plus node:test/viem), all passing | `contracts/*.t.sol`, `test/HopeCoin.ts` |
...
| Local deployment rehearsal (Ignition) | `ignition/modules/HopeCoin.ts` |
...
| **Full launch dress rehearsal** — real Uniswap V2 stack deployed locally; deploy → seed 10M HOPE/100 ETH pool → buyer swap (price verified incl. 0.3% fee) → renounceOwnership, all green | `scripts/rehearse-launch.ts` |
...
Engage a crypto-literate lawyer on: what HOPE is for, how it's distributed,
...
npx hardhat ignition deploy ignition/modules/HopeCoin.ts --network mainnet
...
- [ ] The HOPE/ETH ratio you pass sets the launch price — sanity-check it.
