...
| Item | Value |
| --- | --- |
| Files in scope | `contracts/Card.sol` |
...
| Deployment target | Ethereum mainnet, via Hardhat Ignition (`ignition/modules/Card.ts`) |

## Intended behavior (the spec to audit against)

1. Standard ERC-20 (EIP-20) semantics, exactly as inherited from
   OpenZeppelin `ERC20` — name "Card", symbol "CARD",
   18 decimals.
...
- **TypeScript tests** (node:test + viem) in `test/Card.ts`.
