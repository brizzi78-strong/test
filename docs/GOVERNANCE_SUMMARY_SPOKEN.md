# CARD — Governance and Money. Spoken Summary.

*A plain-language version of the governance and founder economics analysis, written to be
listened to rather than read. No tables, no addresses, no file names. About fifteen minutes.*

---

This is a summary of the decisions in front of you on the coin. Who should be on the board.
What happens if you hold one hundred million of the supply. Whether to charge a three
percent fee on trades. And how you would actually pay yourself. I will take them one at a
time.

Start with the one idea that everything else hangs on.

The coin has no real market value yet. At the launch numbers you described, the whole thing
is worth about twenty thousand dollars on paper. There is no trading crowd, no user base,
no advantage over any other small token. What it does have, and what nothing else in this
category has, is that every claim it makes can be checked by a stranger. The supply can
never grow. The trading pool is locked. Nobody owns the contract. And the coin page tells
people, in plain words, not to buy something they cannot afford to lose.

That checkability is the whole product. So the test for every decision below is simple.
Does it add to that, or spend it?

Now the numbers, so we are talking about the same thing.

Total supply is two hundred fifty million coins. You put in about ten thousand dollars of
your own money. Say eight thousand of that goes into the trading pool and two thousand
covers the lock, the network fees, the legal work, and the accounting. Under your revised
plan, one hundred million coins go into the pool, one hundred million you hold personally,
and fifty million sits in the treasury.

Here is what eight thousand dollars in a trading pool actually feels like. A one hundred
dollar purchase moves the price up about two and a half percent. A five hundred dollar
purchase moves it about thirteen percent. A one thousand dollar purchase moves it about
twenty seven percent. And it works the same way going down. One person selling a thousand
dollars worth drops the price by roughly the same amount.

That is a thin pool. It is survivable, and it is honest, as long as you do not promise
otherwise. But it means one thing you should hold onto: if the price jumps in the first
week, that is not the market telling you the idea is working. It is the market telling you
there is almost no money in the pool.

Now, the board.

You've settled on three people. Three is an odd number, so nothing deadlocks. Two is not a
board, it is a standoff. Five would have been stronger on paper, but you cannot recruit five
unpaid people to put their names on a coin, and five names where two never show up is worse
than three people who actually do the work.

But here is the important distinction, and it is easy to miss. Three people and three keys
are two different questions.

The giving committee is three people. You, a hospice or palliative care professional, and
one more. They decide which families get money. That money comes from book revenue, not
from the coin, which means none of them ever touch a wallet, hold a coin, or appear on the
coin page at all. Those are the easy seats to fill, because there is no crypto in the ask.

The treasury wallet is three keys, and it takes two of them to move anything. You, a paid
reviewer, and one person who is comfortable with crypto. Never two keys, because two can
deadlock, and if either one is lost the money is gone forever. The third key is as much
about survival as oversight.

The two groups only overlap in you. And a key holder does not have to be a public board
member, which is exactly what makes that role fillable.

Why would anyone say yes to any of this? There is no money in it and no coins, so the offer
has to be something else, and it is. Most volunteer board seats are advisory theater. You
attend, you give an opinion, somebody else decides. This one comes with a key and a vote. A
person can actually stop a spend. For the right person, being told you will help decide
which grieving families get money, and you can block me, is a better offer than a few
hundred dollars a meeting.

The thing that actually closes it, though, is the exit. Four hours a month. One year. Resign
any time, and the resignation letter gets published exactly as written, nothing edited out.
People turn these things down because they are afraid of getting stuck defending something
they stopped believing in. Tell them they can walk out the front door and say why, and that
fear goes away.

Now, the crypto problem, because it is real. Serious professionals do not want their names
next to a cryptocurrency, and being unpaid makes it worse, because an unpaid public
association reads as an endorsement while a paid engagement reads as a job. That is not
them being squeamish. That is them being right.

The fix is not better wording, it is the structure above. The giving committee has nothing
to do with the coin, so a chaplain can join it with no exposure at all. And for the hard
role, the independent financial one, pay for it instead of asking for a favor. An accountant
who will not lend his name to a coin for free will take a paid engagement to review your
treasury every quarter, because a professional engagement is not an endorsement and he can
say so to anyone who asks.

On that, you have a name. Jeff Middleton, a C P A, at around fifteen hundred dollars a
year. And he is your father in law, which changes what the money buys. Paying a stranger
works because it turns an endorsement into a job. Family was never declining for
reputational reasons, so the fee does not buy that, and no fee at any price buys the thing
the role exists for, which is judgment from outside your family. He is genuinely valuable as
a paid accountant and as a second signature on the wallet. A family key protects the money
from theft and lost devices. It does not protect against a decision you both think is fine.
Say on the ledger that he is family and what he is paid, and never call him independent.

That number is defensible but honestly below market. C P A time usually runs a hundred fifty
to four hundred dollars an hour, so fifteen hundred buys somewhere between four and ten
hours, which is about right for four quarterly reviews plus being reachable. Bill it three
seventy five a quarter rather than one lump, so there is a record each time the work
actually happened. Pay it from the company straight to his firm and post it on the ledger.

What is in it for him. It is an engagement, not an endorsement, and that is the whole point.
It is small and defined and he can end it. If he already does your books it is easy money on
work he half knows already. And crypto treasury work is a niche that accountants are getting
asked about more and more, and a twenty thousand dollar treasury is a very forgiving place
to learn it.

Two more cautions. Do not call his work an audit or a review. Those words mean specific things in
accounting and carry requirements he may not be able to meet here. Call it treasury
oversight.

Third, holding a key to the wallet may be a problem for him professionally, because having
custody of a client's assets is a real independence issue for accountants and his firm may
have rules about it. Ask him to clear both the engagement and the key with his firm before
he says yes. If the key is a problem, that is easy. He does the oversight, and the third key
goes to somebody else.

One last thing on the name. The coin has had three names. It started as Cardinals Promise,
was briefly Mager Coin, and has landed on its final name: Card,
ticker C P seventeen. Nothing was ever deployed under the earlier names, so every change was
free, and after launch none of them would have been, so this one is the last. The new name
lines everything up. The coin is C P seventeen, the company is C P seventeen L L C, and the
website is c p seventeen dot org. One name, one site, one address. And C P now stands for
Card Platform, not for the book, which finishes the separation between the coin and the
memoir. The name is plain on purpose. It has no story to defend, and the mission lives in
the giving and the ledger instead of the branding. Jeff Mager stays eligible for the board,
because the only thing that ever ruled him out was the coin carrying his name.

When you interview candidates, there is one question that works better than any other. Ask
them what would make them resign. A good answer comes fast and is specific. If you moved
money without telling me. If the gift ledger stopped being published. A vague answer, or I
cannot imagine that ever happening, means they have not taken the seat seriously and they
will not be a constraint on you when you need one.

Two rules on who not to seat. Nobody who holds the coin, because then they are a trader
with a vote. And nobody who cannot say no to you. Warmth is not the qualification here.
Friction is.

Pay board members in cash or not at all. A few hundred dollars a meeting, or just expenses.
Never in coins. And note that five people at a few hundred dollars a meeting is a few
thousand dollars a year, which the coin company does not have. So for now it is expenses
only, and you should say that plainly when you ask.

On the company itself. A separate new limited liability company is the right call, and the
separate part is doing most of the work. The coin is the riskiest thing you are building.
If someone sues over it, you want that to hit the company and stop there. Not the book.
Not the book's royalties. Not the other software. Putting the memoir in the same entity as
the token would mean risking your book to protect a twenty thousand dollar experiment,
which is a trade nobody would make on purpose.

Two things to settle before anyone deploys anything. First, who deploys. The address that
launches the coin is permanent and public, and it is the first thing anyone uses to decide
who is behind this. Second, how the ten thousand dollars gets in. It should go in as a
documented capital contribution, into the company's own bank account, with its own tax ID.
Not you personally paying bills on the company's behalf. Mixing personal and company money
at the very first transaction is the single most common way a company's legal protection
gets stripped away later.

On the entity name. You had considered calling the company Cardinal, or Cardinals Platform.
Renaming the coin settles most of that, but the company still needs a name and the advice is
the same: make it boring. The company name is the one place where being distinctive buys you
nothing, and a complaint about the coin should not arrive carrying a name you care about.

For what it is worth, the reason I would have steered you off Cardinal was never the
baseball or football teams. It was Cardinal Health, a Fortune twenty healthcare company,
because your project is hospice adjacent, which is their exact territory. Cardinal is also
crowded in financial services.

So my suggestion is C P seventeen L L C for the company, which already matches the web
address you own, with Card as the public name of the coin. And notice the good side
effect. With the coin called Card and the company called C P seventeen, the book's
name appears nowhere in the coin's legal identity or its public one. That is a stronger
version of the separation you formed the company to get.

Before you file, do three cheap things in this order. A state name availability search,
which is free and takes minutes. A trademark search covering software, financial services,
and charitable or care services. And a domain check. A trademark screen costs a small
fraction of what a rebrand costs after launch, and after launch the name is attached to a
contract address that can never be edited.

Now the one hundred million you want to hold.

The idea is fine. The way you hold it is what decides whether it helps you or hurts you.
Sitting in your own wallet, it looks exactly like a pending dump. Forty percent of all
coins, in one wallet, that could move any second, against a pool with eight thousand
dollars in it. Every screening tool will flag it. Every skeptical buyer checks that first.

The fix costs you nothing you actually want. Put it in a lock. Two to four years, releasing
gradually, publicly verifiable. Publish the schedule before launch, not after somebody
asks. If you genuinely intend to hold, then a lock takes away nothing at all, and it turns
your single biggest liability into your second best trust signal.

Now the three percent fee. My recommendation is do not do it. Not on principle. On
arithmetic.

At realistic trading volume for a coin this size, three percent earns somewhere between two
thousand and eleven thousand dollars a year. To pay yourself even forty thousand, you would
need one point three million dollars of trading every year. That is thirty six hundred
dollars of trading every single day, forever, against a pool with eight thousand dollars in
it. That does not happen.

And look at what it costs. Your coin page currently says the only fee is the small one the
exchange charges, that you did not add it, and that it does not go to a wallet you can
reach into. A three percent fee paid to you makes that paragraph false. It also requires
writing new, more complicated, unaudited code, when the entire point of the current
contract is that it is boring and does nothing clever. It cannot coexist with giving up
ownership of the contract, because a fee needs someone able to maintain it, and giving up
ownership is the strongest trust signal you have.

And the legal piece. The current design was built specifically so that you are not raising
money and not profiting from other people's trading. A three percent cut of every trade
puts you back on the wrong side of both. It also gives you an obvious financial reason to
promote the price, which is the exact thing you have been careful not to do.

So, how do you get paid.

First, the word salary is wrong. For a single member company, you cannot put yourself on
payroll. There is no paycheck and no W two. What you take is called an owner's draw, which
is just a transfer from the business account to your personal account. Three things people
get wrong about it.

A draw is not a business expense, so it does not lower your tax. You are taxed on the
company's profit whether you take the money out or leave it in the account, so taking
nothing does not mean owing nothing. And nothing is withheld, so on top of income tax you
owe self employment tax of about fifteen percent, and you pay it yourself in four quarterly
installments. Set aside twenty five to thirty five percent of profit as it comes in.
Missing those quarterly payments is the expensive first year mistake.

A real salary only exists if you elect to be taxed as an S corporation, and that starts
making sense somewhere around fifty thousand dollars of steady profit. You are not there.
When you get there, it belongs to whichever business actually earns the money, not to the
coin company.

Now the harder question, which is where the money would even come from.

Look at what the coin company would actually have. Your ten thousand dollars, which is
already spoken for by the pool and the launch costs. Paying yourself out of your own
contributed capital is just moving money between your own pockets. Fifty million coins in
the treasury, worth about four thousand dollars, which cannot be sold into an eight
thousand dollar pool without wrecking it. And fee income, which we just agreed should not
exist.

So the answer is that the coin cannot pay you. Not this year. The money that can pay you is
the book, the speaking, and the software. That is the real source, and it always was.

There is a catch there worth naming. You are forming the separate company to keep token
risk away from the book. But if you route book royalties through that company in order to
pay yourself from it, you have just pulled the book's income back inside the risky entity.
So keep them apart. Book money stays out. The coin company holds the coin, the treasury,
the website, and the giving program, and it pays you nothing for now.

What it can do right now is cover operational costs, and this is cleaner than paying
yourself anyway. Best version, the company pays vendors directly. The lock service, the
audit, the network fees, the registered agent, the lawyer, the accountant. Company account
straight to the vendor, no money passing through your hands at all. Second best, the
company reimburses you for costs you fronted, with receipts and a written business reason,
which is not income to you. Only third, and needing the most explanation, is paying
yourself for your time.

Work down that list, not up. Almost everything you need money for in year one is in the
first category.

And here is the constraint that governs all of it, including the gifts. Every one of these
things needs actual dollars, and the treasury holds coins, not dollars. So the treasury is
not a budget. It is a claim on a budget that does not exist yet. Which means do not promise
a giving schedule, a spending plan, or a salary that assumes you can spend it. Your coin
page already does this right by refusing to name a charity percentage it might have to walk
back. Apply exactly that discipline to your own pay.

So, what to do, in order.

Clear the name. Form the company, get the tax ID, open its bank account, and document the
ten thousand dollars going in. Open the shared wallet so no money moves without a second
signature. Start it at two of three, and move it to three of five once all five seats are
actually filled. Do not build a three of five wallet around three people, because a
threshold you cannot meet is a treasury you cannot use. Screen your three candidates and recruit an outsider for the fifth
seat, then assign seats. Fill the constraint seat first, with someone who has no tie to you, because
everything gets easier once one person can tell you no. Then the hospice seat. And adopt
the tie fails rule before the first meeting. Drop the fee. Lock your one
hundred million and publish the schedule. Update the website so it describes what is
actually true on day one. Put the giving policy and your compensation agreement in writing
before either one is needed. Then launch. Then publish, every quarter, the gifts, the
spending, the meeting notes, and anything you were paid.

One last thing. None of this is legal or tax advice. The securities questions belong with a
lawyer and the pay and company questions belong with an accountant. But you will walk into
both of those conversations knowing what you want, which makes them much shorter and much
cheaper.
