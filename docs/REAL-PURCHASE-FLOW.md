...
4. The site shows the HOPE exchange rate, provider fees, the network fee, and
...
- **Create a HOPE/USDC pool** — one hop, simplest quote. Costs: the launch ETH
  has to become USDC, and WETH-pair buyers (the `how-to-buy.html` path) then
  route through two hops instead.
- **Keep HOPE/WETH and route USDC → WETH → HOPE** — two hops, so Uniswap's
  0.3% is paid on both legs and slippage compounds across two thin pools.
...
### 2. The quote must be honest about every cost

The "expected amount received" in step 4 must subtract, in this order: the
on-ramp provider's fee, Uniswap's 0.3% pool fee, price impact from the pool's
depth, and the network gas cost. HOPE itself takes nothing — it is a plain
ERC-20 with no transfer fee — so no fee-on-transfer router variants are needed
and the quote is the pool math plus the provider's cut, nothing hidden.
...
- The company **issued the token**. It takes no fee from trades (the transfer
  fee was removed before launch), which helps — but issuing the asset and
  running the storefront for it is still the core fact.
...
- `docs/LEGAL-BRIEFING.md` notes the current design "deliberately weakens
  several Howey prongs." A first-party purchase funnel pushes back against
  that; removing the fee helps, but the funnel itself is the question.
...
example: no first-party funnel, or geographic restrictions).

## What stays true regardless

The company never holds funds, keys, or ID documents. Every disclosure the
project already publishes — the risk disclosures in the app, the published
treasury address, the unlocked founder position — applies to this flow too, and
the cost arithmetic shown at step 4 must match what the pool actually does.
