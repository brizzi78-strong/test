...
import type { OffboardingService } from '../service/offboardingService.ts';
...
function taskType(raw: string): OffboardingTaskType {
  if (!OFFBOARDING_TASK_TYPES.includes(raw as OffboardingTaskType)) {
    throw new ValidationError(`unknown task type: ${raw}`);
  }
  return raw as OffboardingTaskType;
}

export function buildRoutes(service: OffboardingService): Route[] {
  const routes: Array<[string, string, Handler]> = [
    ['GET', '/health', () => ok({ status: 'ok' })],
    [
      'GET',
      '/meta',
      () => ok({ service: 'offboarding', reasons: SEPARATION_REASONS, taskTypes: OFFBOARDING_TASK_TYPES }),
    ],
    ...
    ['POST', '/cases', ({ body }) => created(service.createCase(asObject(body) as never))],
    [
      'GET',
      '/cases',
      ({ query }) =>
        ok(
          service.listCases({
            ...
            status: (query.get('status') as CaseStatus | null) ?? undefined,
          }),
        ),
    ],
    ['GET', '/cases/:id', ({ params }) => ok(service.getCase(params.id))],
    [
      'POST',
      '/cases/:id/tasks/:type/complete',
      ({ params, body }) => ok(service.completeTask(params.id, taskType(params.type), asObject(body) as never)),
    ],
    [
      'POST',
      '/cases/:id/tasks/:type/na',
      ({ params, body }) => ok(service.markTaskNotApplicable(params.id, taskType(params.type), asObject(body) as never)),
    ],
    ['POST', '/cases/:id/cancel', ({ params, body }) => ok(service.cancelCase(params.id, asObject(body ?? {}) as never))],
  ];
...
export function createRequestListener(service: OffboardingService) {
