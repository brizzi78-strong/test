...
export interface SmtpConfig {
  ...
  user?: string;
  password?: string;
  /** Envelope + From address, e.g. "no-reply@thecardinalspromise.blog". */
  from: string;
  fromName?: string;
...
export class SmtpNotifier implements Notifier {
  ...
  private buildMessage(email: OutboundEmail): string {
    ...
    const boundary = `=_aidfinder_${randomUUID().replace(/-/g, '')}`;
...
export function smtpFromEnv(env: NodeJS.ProcessEnv = process.env): SmtpNotifier | undefined {
  if (!env.SMTP_HOST || !env.AIDFINDER_MAIL_FROM) return undefined;
  ...
  return new SmtpNotifier({
    ...
    from: env.AIDFINDER_MAIL_FROM,
    fromName: env.AIDFINDER_MAIL_FROM_NAME,
  });
}
