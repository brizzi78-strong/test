...
import type { Company, Course, Enrollment, Learner } from '../domain/types.ts';
import type { Collection, Store } from './store.ts';

const TABLES = ['companies', 'learners', 'courses', 'enrollments'] as const;
...
export function createSqliteStore(path: string): SqliteStore {
  ...
  return {
    ...
    learners: new SqliteCollection<Learner>(db, 'learners'),
    courses: new SqliteCollection<Course>(db, 'courses'),
    enrollments: new SqliteCollection<Enrollment>(db, 'enrollments'),
    close: () => db.close(),
  };
}
