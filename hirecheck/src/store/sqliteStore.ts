 ...
 * table holding the JSON-serialized entity; predicate filtering happens in JS
 * to keep the Collection contract identical.
 */
...
const TABLES = ['companies', 'candidates', 'packages', 'orders'] as const;
...
export function createSqliteStore(path: string): SqliteStore {
  ...
  return {
    ...
    packages: new SqliteCollection<ScreeningPackage>(db, 'packages'),
    orders: new SqliteCollection<ScreeningOrder>(db, 'orders'),
    close: () => db.close(),
  };
}
