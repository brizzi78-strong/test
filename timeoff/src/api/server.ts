/**
 * HTTP server wiring: assemble a TimeOffService with a store (in-memory or the
 * durable SQLite store, selected by env) and expose it over HTTP.
 */
...
/**
 * Select a store from the environment: `TIMEOFF_DB=/path/to/data.db` uses the
...
export function storeFromEnv(env: NodeJS.ProcessEnv = process.env): Store {
  return env.TIMEOFF_DB ? createSqliteStore(env.TIMEOFF_DB) : createInMemoryStore();
}
...
export function createApp(store: Store = storeFromEnv()): AppServer {
  const service = new TimeOffService({ store });
