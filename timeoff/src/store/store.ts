...
import type { Balance, Company, Employee, LeavePolicy, TimeOffRequest } from '../domain/types.ts';

export interface Store {
  companies: Collection<Company>;
  employees: Collection<Employee>;
  policies: Collection<LeavePolicy>;
  balances: Collection<Balance>;
  requests: Collection<TimeOffRequest>;
}
...
export function createInMemoryStore(): Store {
  return {
    ...
    employees: new InMemoryCollection<Employee>(),
    policies: new InMemoryCollection<LeavePolicy>(),
    balances: new InMemoryCollection<Balance>(),
    requests: new InMemoryCollection<TimeOffRequest>(),
  };
}
