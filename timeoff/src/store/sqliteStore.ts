...
const TABLES = ['companies', 'employees', 'policies', 'balances', 'requests'] as const;
...
export function createSqliteStore(path: string): SqliteStore {
  ...
  return {
    ...
    policies: new SqliteCollection<LeavePolicy>(db, 'policies'),
    balances: new SqliteCollection<Balance>(db, 'balances'),
    requests: new SqliteCollection<TimeOffRequest>(db, 'requests'),
    close: () => db.close(),
  };
}
