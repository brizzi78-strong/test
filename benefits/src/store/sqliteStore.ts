...
import type { BenefitPlan, Company, Employee, Enrollment } from '../domain/types.ts';
import type { Collection, Store } from './store.ts';

const TABLES = ['companies', 'employees', 'plans', 'enrollments'] as const;
...
export function createSqliteStore(path: string): SqliteStore {
  ...
  return {
    ...
    plans: new SqliteCollection<BenefitPlan>(db, 'plans'),
