/**
 * HTTP server wiring: assemble an AccountingService with a store and expose it
 * over HTTP.
 */
...
/**
 * Select a store from the environment: `ACCOUNTING_DB=/path/to/data.db` uses the
...
export function storeFromEnv(env: NodeJS.ProcessEnv = process.env): Store {
  return env.ACCOUNTING_DB ? createSqliteStore(env.ACCOUNTING_DB) : createInMemoryStore();
}
