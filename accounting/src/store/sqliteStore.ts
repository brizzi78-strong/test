...
const TABLES = ['companies', 'accounts', 'customers', 'invoices', 'expenses'] as const;
...
export function createSqliteStore(path: string): SqliteStore {
  ...
  return {
    ...
    invoices: new SqliteCollection<Invoice>(db, 'invoices'),
    expenses: new SqliteCollection<Expense>(db, 'expenses'),
    close: () => db.close(),
  };
}
