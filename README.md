...
| **Cardinal ERP — SAP-style supply chain** | `sap/` | Standalone ERP core: materials, purchasing (PO → goods receipt → three-way-matched vendor invoice with payment blocks), moving-average stock valuation, and sales (order → ATP check → delivery → billing) with SAP-style document flow (see `sap/README.md`) |
