...
| **Invest — trading app** | `invest/` | A usable multi-user trading app: sign up / log in, then your own paper portfolio, watchlist, orders, and buy/sell — a backend-for-frontend over the Trading service with every call scoped server-side to the logged-in user's account (see `invest/README.md`) |
