...
| **AidFinder — free money for college** | `aidfinder/` | Automated college-funding finder: a five-minute student profile matched against a curated catalog of 24 grants, scholarships, tax credits, and employer/military benefits — with a computed Pell estimate, a near-miss report (what would unlock more money), a deadline-ordered action plan with iCalendar export, and an application tracker. Finds and schedules the money; never submits applications for you (see `aidfinder/README.md`) |
