...
| **Drug Discovery — cheminformatics workbench** | `drugdiscovery/` | Front-of-pipeline small-molecule triage: register a biological target, design candidate molecules as physicochemical descriptor profiles, score their drug-likeness live against the published Lipinski / Veber / lead-likeness rules, record screening potencies, and rank the candidates ("advance these first"). Reasons about molecules as numbers, not recipes — no synthesis routes; a triage aid, not a validated ADMET model (see `drugdiscovery/README.md`) |
