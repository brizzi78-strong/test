...
| **Hope Coin (HOPE) token** | `contracts/`, `test/`, `verification/`, `site/` | Fixed-supply ERC-20 with a complete launch kit ([below](#cardinals-promise-card-token)) |
...
scripts/add-liquidity.ts                   # create/seed the Uniswap V2 HOPE/ETH pool
...
secrets, then verify the deployed address — `HopeCoin` takes no
