...
**Cardinals Promise (CP17)** — a fixed-supply ERC-20 on Ethereum. The full
...
```
contracts/CP17.sol             # the token (OpenZeppelin ERC20 + Ownable)
contracts/CP17.t.sol           # Foundry-style Solidity tests (forge-std)
contracts/CP17Invariants.t.sol # stateful fuzz/invariant suite (handler-based)
test/CP17.ts                   # TypeScript tests (node:test + viem)
...
scripts/add-liquidity.ts                   # create/seed the Uniswap V2 CP17/ETH pool
ignition/modules/CP17.ts       # Hardhat Ignition deployment module
...
secrets, then verify the deployed address — `CP17` takes no
