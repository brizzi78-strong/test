...
describe("Nolax", async function () {
  const { viem } = await network.create();

  it("mints the full fixed supply to the deployer", async function () {
    const token = await viem.deployContract("Nolax");
  ...
  it("has the expected metadata", async function () {
    ...
    assert.equal(await token.read.name(), "Nolax");
    assert.equal(await token.read.symbol(), "NOLAX");
