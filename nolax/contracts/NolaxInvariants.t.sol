...
import {Nolax} from "./Nolax.sol";

/// @notice Fuzz handler for Nolax invariant testing. The invariant runner
...
contract NolaxHandler {
    Vm private constant vm =
        Vm(address(uint160(uint256(keccak256("hevm cheat code")))));

    Nolax public immutable token;
    ...
    constructor(uint256 numActors) {
        ...
        for (uint256 i = 0; i < numActors; i++) {
            address actor = address(uint160(0xA0FA0000 + i));
            actors.push(actor);
