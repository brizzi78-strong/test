...
///         accrues to a fixed treasury address on transfers between
///         non-treasury addresses. The entire 250M supply is
...
///      the treasury are fee-exempt so treasury funding, pool seeding, and
///      approved treasury distributions arrive whole and do not self-tax.
contract CardinalsPromise is ERC20, Ownable {
    uint256 public constant TOTAL_SUPPLY = 250_000_000 * 1e18;

    /// @notice Non-treasury transfer fee in basis points, fixed forever.
    uint256 public constant FEE_BPS = 200; // 2%

    /// @notice Where every collected fee goes. Set once, never changeable.
    ...
    constructor(address treasury_) ERC20("Cardinals Promise", "CARD") Ownable(msg.sender) {
        ...
        require(treasury_ != msg.sender, "CardinalsPromise: treasury is deployer");
        treasury = treasury_;
