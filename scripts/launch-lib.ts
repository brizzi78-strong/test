...
export function loadLaunchConfig(url: URL): LaunchConfig {
  ...
  if (!raw.token) {
    throw new Error(
      "launch.json: fill in \"token\" with the deployed Presence address first.",
    );
  }
  if (!raw.treasury) {
    throw new Error(
...
export const fmt = (wei: bigint): string =>
  `${Number(formatEther(wei)).toLocaleString("en-US")} HERE`;
...
export function describeStage(
  ...
  if (s.name !== "Presence" || s.symbol !== "HERE") {
    problems.push(
      `token name/symbol is ${s.name}/${s.symbol}, expected "Presence"/HERE — is the token address right?`,
    );
  }
...
// Checklist step 3: send exactly 50M HERE to the treasury. Refuses to run
