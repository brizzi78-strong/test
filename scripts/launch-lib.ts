...
export function loadLaunchConfig(url: URL): LaunchConfig {
  ...
  if (!raw.token) {
    throw new Error(
      "launch.json: fill in \"token\" with the deployed MagerCoin address first.",
    );
  }
  if (!raw.treasury) {
    throw new Error(
...
export const fmt = (wei: bigint): string =>
  `${Number(formatEther(wei)).toLocaleString("en-US")} MAGR`;
...
export function describeStage(
  ...
  if (s.name !== "Mager Coin" || s.symbol !== "MAGR") {
    problems.push(
      `token name/symbol is ${s.name}/${s.symbol}, expected "Mager Coin"/MAGR — is the token address right?`,
    );
  }
...
// Checklist step 3: send exactly 50M MAGR to the treasury. Refuses to run
