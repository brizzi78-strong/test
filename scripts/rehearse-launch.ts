 ...
 *
 *   1. deploy HopeCoin (full 250M supply to the deployer)
 *   2. create + seed the HOPE/WETH pool
 *   3. execute a buyer swap (ETH -> HOPE) to prove the market works
...
async function main() {
  ...
  console.log(`HOPE deployed at ${token.address}`);
  ...
  hash = await router.write.addLiquidityETH(
    [token.address, POOL_HOPE, POOL_HOPE, POOL_ETH, deployer.account.address, deadline],
    { value: POOL_ETH },
  );
  await publicClient.waitForTransactionReceipt({ hash });
  ...
  console.log(`Buyer swapped 1 ETH -> ${formatEther(bought)} HOPE`);
