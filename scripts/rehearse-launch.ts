/**
 * Full offline dress rehearsal of the MAGR launch sequence against a local
 ...
 *
 *   1. deploy MagerCoin (full 250M supply to the deployer)
 *   2. create + seed the MAGR/WETH pool
 *   3. execute a buyer swap (ETH -> MAGR) to prove the market works
...
const POOL_MAGR = parseEther("10000000"); // 10M MAGR into the pool
...
async function main() {
  ...
  // --- Step 1: deploy MAGR --------------------------------------------
  const token = await viem.deployContract("MagerCoin");
  console.log(`MAGR deployed at ${token.address}`);
  const totalSupply = (await token.read.totalSupply()) as bigint;
  console.log(`  totalSupply: ${formatEther(totalSupply)} MAGR`);

  // --- Step 2: seed the MAGR/WETH pool --------------------------------
  let hash = await token.write.approve([router.address, POOL_MAGR]);
  await publicClient.waitForTransactionReceipt({ hash });

  const deadline = BigInt(Math.floor(Date.now() / 1000) + 20 * 60);
  hash = await router.write.addLiquidityETH(
    [token.address, POOL_MAGR, POOL_MAGR, POOL_ETH, deployer.account.address, deadline],
    { value: POOL_ETH },
  );
  await publicClient.waitForTransactionReceipt({ hash });

  const pair = await factory.read.getPair([token.address, weth.address]);
  console.log(`Pool seeded: 10,000,000 MAGR + 100 ETH, pair at ${pair}`);
  console.log(`  implied launch price: 1 ETH = 100,000 MAGR`);

  // --- Step 3: a buyer swaps ETH for MAGR ------------------------------
  ...
  console.log(`Buyer swapped 1 ETH -> ${formatEther(bought)} MAGR`);
