// Exports the solc standard JSON input for MagerCoin.sol so the contract can
...
//
// Writes verification/MagerCoin.solc-input.json and prints the exact compiler
...
const target = "project/contracts/MagerCoin.sol";
...
const outFile = join(outDir, "MagerCoin.solc-input.json");
...
console.log(`Contract to select after upload: ${target}:MagerCoin`);
