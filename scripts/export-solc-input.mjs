// Exports the solc standard JSON input for Presence.sol so the contract can
...
//
// Writes verification/Presence.solc-input.json and prints the exact compiler
...
const target = "project/contracts/Presence.sol";
...
const outFile = join(outDir, "Presence.solc-input.json");
...
console.log(`Contract to select after upload: ${target}:Presence`);
