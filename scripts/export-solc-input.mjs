// Exports the solc standard JSON input for Card.sol so the contract can
...
//
// Writes verification/Card.solc-input.json and prints the exact compiler
...
const target = "project/contracts/Card.sol";
...
const outFile = join(outDir, "Card.solc-input.json");
...
console.log(`Contract to select after upload: ${target}:CARD`);
