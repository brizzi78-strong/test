// Exports the solc standard JSON input for HopeCoin.sol so the contract can
...
//
// Writes verification/HopeCoin.solc-input.json and prints the exact compiler
...
const target = "project/contracts/HopeCoin.sol";
...
const outFile = join(outDir, "HopeCoin.solc-input.json");
...
console.log(`Contract to select after upload: ${target}:HopeCoin`);
