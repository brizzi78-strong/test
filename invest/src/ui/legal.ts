...
<h2>3. The CARD token page</h2>
<p>The CARD page can show links that open <b>Uniswap</b>, a third-party decentralized exchange, pre-filled for a real on-chain swap. Those links remain disabled until the verified contract address is published at cp17.org. If you follow one:</p>
<ul>
  <li>The transaction happens between <b>your own crypto wallet</b> and the Uniswap protocol. Cardinal Trading never holds your funds, tokens, or keys and cannot reverse or refund a transaction.</li>
  <li>CARD itself sends 2% of each transfer between non-treasury addresses to the project's immutable treasury. A buy and a sell each trigger that fee. With two Uniswap charges, the simple round-trip estimate is about 4.5% before gas, slippage, and price impact.</li>
...
<ul>
  ...
  <li>No crypto wallet addresses or keys. If you use a Uniswap link from the CARD page, that activity happens in your own wallet and on the public Ethereum network, outside our systems.</li>
...
const DISCLOSURES = shell(
  ...
  /* html */ `
<div class="card warn"><b>The short version:</b> Cardinal Trading is for practice. Nothing here is investment advice. If you ever trade real money — anywhere — you can lose it, and crypto tokens like CARD can lose <b>all</b> of their value.</div>
...
<p>Listing an instrument in the app — including CARD — is not an endorsement or a recommendation. We are not investment advisers and nothing in the app considers your personal financial situation. Before investing real money, consider speaking with a licensed financial adviser.</p>

<h2>Crypto risk, and CARD specifically</h2>
<ul>
  <li>Crypto tokens are <b>extremely volatile and speculative</b>. Small-pool tokens like CARD can move violently on tiny trades, may have little or no liquidity when you want to sell, and can go to zero.</li>
  <li>The people behind Cardinal Trading and cp17.org created CARD, control its treasury, and hold a large founder allocation. That is a <b>conflict of interest</b> you should weigh even though the addresses are public.</li>
  <li>CARD charges an immutable 2% fee on transfers between non-treasury addresses. Transfers to or from the treasury are exempt. The token fee compounds to 3.96% across a buy and sell; two 0.3% Uniswap charges bring the simple estimate to about 4.5% before gas and market effects.</li>
