...
The program preview is live at <https://memory-care-music-program.onrender.com/>,
deployed from `main` by the repo's `../render.yaml` blueprint (auto-deploy on
every merge). The public entrypoint is [`index.html`](index.html), served by
[`publish-server.mjs`](publish-server.mjs). It frames the self-contained
program preview ([`demo/index.html`](demo/index.html), also openable directly
in any browser) with honest sample data, stable production routes, security
