...
const TABLES = ['facilities', 'residents', 'songs', 'playlists', 'sessions'] as const;
...
export function createSqliteStore(path: string): SqliteStore {
  ...
  return {
    facilities: new SqliteCollection<Facility>(db, 'facilities'),
    residents: new SqliteCollection<Resident>(db, 'residents'),
    songs: new SqliteCollection<Song>(db, 'songs'),
    playlists: new SqliteCollection<Playlist>(db, 'playlists'),
    sessions: new SqliteCollection<SingSession>(db, 'sessions'),
    close: () => db.close(),
  };
}
