...
const TABLES = ['companies', 'requisitions', 'applications'] as const;
...
export function createSqliteStore(path: string): SqliteStore {
  ...
  return {
    ...
    requisitions: new SqliteCollection<JobRequisition>(db, 'requisitions'),
    applications: new SqliteCollection<Application>(db, 'applications'),
    close: () => db.close(),
  };
}
