# HOPE Token Launch Strategy

Launch plan for the HOPE token: 250M fixed supply, Uniswap fair launch, trust-first design.
...
| Into the Uniswap pool | **100M HOPE (40%)** | The tradeable float. Halving it from the original 200M doubles the launch price but does not deepen the book — see the slippage table in `docs/GOVERNANCE_AND_FOUNDER_ECONOMICS.md` |
...
4. **Create the Uniswap V2 pool** with 100M HOPE + 2–5 ETH, via `addLiquidityETH` on the
   router (`scripts/add-liquidity.ts`). This is not a swap — you are depositing both sides
   and thereby *setting* the opening price, not paying one. The HOPE/ETH ratio is the price:
   with 100M in the pool, 3 ETH implies 0.00000003 ETH/HOPE. Check the ratio twice; it is the
