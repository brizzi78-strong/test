...
const TABLES = ['companies', 'employees', 'templates', 'packets'] as const;
...
export function createSqliteStore(path: string): SqliteStore {
  ...
  return {
    ...
    templates: new SqliteCollection<PacketTemplate>(db, 'templates'),
    packets: new SqliteCollection<OnboardingPacket>(db, 'packets'),
    close: () => db.close(),
  };
}
