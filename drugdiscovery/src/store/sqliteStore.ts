...
import type { Compound, Program, Screen, Target } from '../domain/types.ts';
import type { Collection, Store } from './store.ts';

const TABLES = ['programs', 'targets', 'compounds', 'screens'] as const;
...
export function createSqliteStore(path: string): SqliteStore {
  ...
  return {
    programs: new SqliteCollection<Program>(db, 'programs'),
    targets: new SqliteCollection<Target>(db, 'targets'),
    compounds: new SqliteCollection<Compound>(db, 'compounds'),
    screens: new SqliteCollection<Screen>(db, 'screens'),
    close: () => db.close(),
  };
}
