# Rob's Fragments — Raw Memory Bank

*Verbatim (lightly cleaned) voice notes and fragments from Rob. This file is
the source of truth for anchor material — chapters draw from here, never
the other way around. Nothing in this file is invented.*

---

## 2026-07-30 — the Wegovy-era voice note

> This story will be more impactful than the Viagra story because Ozempic
> has completely outshined Viagra, which was one of a kind — and put Pfizer
> on the map. But Novo Nordisk went from being a mid-level insulin company
> to creating Ozempic and being talked about in the media, Oprah, the
> Kardashians. Let me be clear: this was not intended to be a weight-loss
> medication. It was a side effect. We had our own version, which was on
> shortage — Wegovy — and we always stayed on label. But there were times
> where offices would just — "we need samples, we need samples." And then
> finally I kept looking at the numbers and saying: there aren't any new
> scripts, and the staff's getting skinnier. We can't sample here anymore.

**What this anchors (all now TRUE, not stand-in):**
1. Rob was still carrying the bag during the Wegovy shortage / early craze —
   he watched the weight-loss wave from INSIDE the territory. (Timeline
   flag: the craze, media, and Wild West chapters currently frame him as
   already out of pharma — his real departure date decides how those
   chapters' vantage points get rewritten.)
2. The staff-getting-skinnier scene: offices demanding samples, no new
   scripts in the data, the staff visibly losing weight — and Rob cutting
   an office off. His words: "We can't sample here anymore."
3. "We always stayed on label" — Wegovy was the on-label weight-loss
   version and it was on shortage; Ozempic's weight-loss fame was, from the
   rep's seat, a side effect that swallowed the brand.
4. The book's market thesis in his voice: Novo went from mid-level insulin
   company to Oprah and the Kardashians; the story outgrew the Viagra story.

**Accuracy note for drafting:** semaglutide's weight effect was known in
trials and Wegovy was developed deliberately — "it was a side effect" is
true as the *rep's lived frame* (Ozempic was never his weight-loss product;
the cultural use was never the plan he was trained on). Draft it as his
claim from his seat, not as pharmacology.

**Still needed:** which office(s), roughly when, what he actually said to
the office and to his manager, and what happened after the cutoff.

---

## 2026-07-30 — the hallway call, clinically

> The hallway call. It was amazing that people who were on insulin for
> years — still had some pancreatic function — were able to go on a
> once-a-week injection and have A1Cs under [seven — note: Rob said
> "three," which is physiologically impossible; assumed slip, confirm]
> ... diabetes, because of the medication. And the studies were in the
> works, public knowledge, about reducing Alzheimer's disease, preserving
> kidney function, reducing cardiovascular events. Who are these people?

**What this anchors:**
1. The clinical wonder under the hallway question: long-time insulin
   patients with residual pancreatic (beta-cell) function moving to
   once-weekly dosing and reaching control — some coming off insulin.
   (Clinically sound for type 2 with residual function.)
2. The era's emerging science as Rob lived it: kidney preservation (FLOW →
   the 2025 approval), cardiovascular reduction (SUSTAIN-6, SELECT),
   Alzheimer's trials in the works and publicly known (evoke/evoke+).
3. **"Who are these people?"** — the call's real question: which patients
   in this office's panel could this still save. The clinical twin of
   "Who's the patient you're worried about?"

**Confirm:** the A1C figure (said "three" — under seven assumed); whether
patients in his territory actually came off insulin; which studies he
actually discussed on calls (on-label constraints applied — the Alzheimer's
material was public science, not a claim he could sell with).

---

## 2026-07-30 — the first line, the exam, and why they hired him

> First line of the book is gonna be: I didn't know the difference between
> type one and type two diabetes, and I was about to start training. The
> training process was over the course of three months. I had to take a
> diabetes exam. If I didn't score ninety percent or above — I had two
> chances — I would lose my job. So talk about going from naive about
> diabetes to understanding GLP-1s, mechanism of action, pancreatic
> function, glucagon, all the different diabetes medications. There was a
> time when pharmaceutical and medical device companies would hire
> pharmacists only, clinical people, nurses — and what they realized was
> they weren't selling, even though these people were super knowledgeable.
> And so they found: let's take a good salesperson and teach them the
> science. And, wow, it worked.

**What this anchors (all TRUE):**
1. **The book's first line, by Rob's decision:** "I didn't know the
   difference between type 1 and type 2 diabetes, and I was about to start
   training."
2. The training: three months; a diabetes exam requiring ≥90%, two
   chances, job on the line.
3. The syllabus of the transformation: GLP-1 mechanism of action,
   pancreatic function, glucagon, the full diabetes formulary.
4. The industry's hiring history: clinical hires (pharmacists, nurses)
   knew everything and couldn't sell; the pivot — take a good salesperson,
   teach them the science. His verdict, in his cadence: "And, wow, it
   worked." (This answers the panel's earlier question about what it means
   that the industry puts the disease-naive in front of the sick — it was
   the strategy, and Rob is its proof and its self-implication.)

---

## 2026-07-30 — the real Number One story

> The territory was an underperforming territory. It was a territory that
> split, and most of the time these accounts hadn't been hit. It took some
> time, and, you know, there was over two hundred reps in the diabetes
> division — I mean, more than that, I'll just be conservative. But through
> believing, working together and inspiring each other, this little county
> in Santa Rosa County ended up being number one in the country for
> Ozempic sales for the third quarter of two thousand twenty-four. Just
> mind-boggling.

**What this anchors (all TRUE):**
1. The territory: **Santa Rosa County** (Florida Panhandle — confirm), an
   underperforming territory created by a split, full of accounts that
   "hadn't been hit."
2. The scale: 200+ reps in the diabetes division (his conservative number).
3. The result: **#1 in the country for Ozempic sales, Q3 2024.**
4. The frame he chooses: collective — "believing, working together,
   inspiring each other." The little county wins, not just the rep.

**TIMELINE — this settles the book's biggest open item:** Rob was in the
field through at least Q3 2024. The shortage, the craze, the media
whiplash, the compounding Wild West — he lived them all carrying the bag.
Every chapter framed as "after I turned in my bag" (craze, media, Wild
West, and the cookout scene) must be re-pointed. Also: the hallway
chapter's "New Jersey, fall of 2019" office is now uncertain — his
territory geography (NJ vs. Florida, and when) needs his dates.

**Still needed:** when he took the territory; the split story; who
"together" was (counterpart reps, the district?); the moment he found out
about Q3; and his actual pharma departure date.

---

## 2026-07-30 — the rival rep in Pace, Florida

> I had a lunch scheduled with an important physician in Pace, Florida,
> and I'm bringing in lunch at the time. And I see a rep in the office,
> and I said, oh, what product do you have? And he says, compounded
> Ozempic. And I sat back, and I thought to myself: he's actually an
> illegal drug dealer. Because this is a patented medication.

**What this anchors (TRUE):**
1. The Wild West chapter's real opening scene — in-territory, while Rob
   carried the bag: a compounded-semaglutide operation running its own
   reps into the same offices, detailing against the real thing.
2. The location: Pace, Florida — Santa Rosa County, consistent with the
   #1-territory anchor.
3. Rob's exact thought, in his words: "he's actually an illegal drug
   dealer... because this is a patented medication."
4. Confirms again: the compounding era happened *during* his selling
   years, not after.

**Drafting note:** his gut verdict ("illegal") sits alongside the legal
nuance the chapter already carries (shortage-era 503A/B compounding was
lawful in a narrow lane; much of what sold as "compounded Ozempic" wasn't
in that lane, and the name itself — Ozempic is a brand — was never
compoundable). Keep the gut verdict as his lived reaction, keep the
nuance as narration; don't let either erase the other.

**Still needed:** what happened next in that office — did you say anything
to the rep, the physician, your manager, the company? Novo's public
lawsuits against compounders/med spas may intersect your dates.

---

## 2026-07-30 — the $170 coffee (home health budget shock)

> In home health, we spent about two hundred dollars a month. And the
> first time I had picked up coffee — there was an office that we did an
> in-service at, and I went to pick it up at Starbucks — and they said,
> that'll be a hundred and seventy dollars. And I said, oh no, no, no,
> you must be mistaken, that can't be right. And come to find out, that's
> the kind of budget that we had.

**What this anchors (TRUE):**
1. The transition chapter's seed — from pharma (lunches bought weekly,
   Universal Studios closed for two hours) to home health: ~$200/month
   total budget, and one Starbucks in-service order was $170 of it.
2. His exact reaction at the register: "Oh no, no, no. You must be
   mistaken."
3. The two economies of care, in one coffee order — the marketing money
   follows the margin, not the need. (The industry that had everything
   sold to the healthy-ish; the one with $200 a month serves the dying.)

**Home for it:** the unwritten transition chapter — working title "Two
Hundred Dollars a Month" — leaving the bag, entering home health, the
budget shock, the road to hospice. This is the chapter between "The
Keynote"/Part II and Part III that the book doesn't have yet.

**Still needed:** when you left Novo and why (the actual decision), the
home-health role and company form, and whether the coffee memory has a
date/office attached.

---

## 2026-08-31 — "Start 8/2021" (the Novo start date)

**ANCHOR: Rob started at Novo Nordisk in August 2021.** The timeline key.
Cascade:

- The arc is **three years**, not five: not-knowing (Aug 2021) → training
  (fall 2021, the 90% exam) → territory → **#1 in the country, Q3 2024**.
  Faster arc = stronger tagline. Jacket updated.
- He never sold the quiet drug. Wegovy launched June 2021, two months
  before he started; the shortage and craze span his entire tenure. Part I
  is a man learning the job *inside* the gold rush.
- The hallway scene ("a year and a half in") re-dates to **winter/early
  2023** — peak shortage era; coats-on-laps works. The drafted "New
  Jersey" office is now doubtful (territory was Florida by the #1 era) —
  geography is Rob's to fix.
- Pen-and-pill's "almost two years in, they handed me the pill" is now
  wrong — Rybelsus (2019) was already in the bag when he arrived. That
  chapter needs reframing: both doors in the bag from day one.
- The Brady keynote timing problem **resolves**: a 2023–24 national
  meeting fits both his tenure and Brady's post-retirement keynote
  circuit. The earlier panel's impossibility flag is lifted; date still
  needs pinning.
- Samples-no-credit's rationing framing: his tenure (2021→) overlaps the
  tail of the insulin-pricing crisis, pre-2023 cuts — survives, dates
  tightened.
- AbbVie / C.R. Bard were evidently **before** Novo and non-diabetes —
  which is exactly why he could arrive at Novo not knowing type 1 from
  type 2 while already being a good salesperson. Consistent with the
  hire-the-salesperson chapter.
- Still open: **departure date** (after Q3 2024), and the order/years of
  the AbbVie and Bard chapters of his career.

---

## 2026-08-31 — Dave was a friend; and "where are the numbers"

> The friend who lost his son, Diabetes Dave.

> And the[y] said where are the numbers.

**Anchored:** Dave was Rob's **friend** — not just a colleague. The Dave
chapter warms accordingly ("He was my friend.").

**AMBIGUOUS — needs Rob's word before it enters any chapter.** "They said
where are the numbers" has three possible readings:
1. **The Dave reading (devastating if true):** while Dave was grieving his
   son, the company/managers still asked "where are the numbers." If this
   is what happened, it may be the most important sentence in the Dave
   chapter — the counted ledger's blindness, aimed at a grieving father.
2. **The sample-cutoff reading:** Rob looking at the skinnier-staff
   office's data — "where are the numbers?" (no new scripts).
3. **The territory reading:** management pressing the underperforming
   split territory early on — "where are the numbers?" — before the climb
   to #1.
Rob: which one? (Or more than one — the phrase could echo across
chapters, which the book could use deliberately.)

---

## 2026-08-31 — "Rob, you made someone uncomfortable for hugging them"

> He said, Rob, you made someone uncomfortable for hugging them.

**Logged verbatim; NOT drafted into any chapter yet** — it involves a real
person's discomfort, and the book must never dismiss that or read as
grievance. Questions before this goes anywhere:
1. Who said it (manager? HR?), when, and in which world — pharma or the
   later jobs?
2. Who was hugged, and what was the context (an office staffer? a grieving
   family? a celebration?)? Would they be identifiable in any telling?
3. How do you hold it now — including the honest possibility that the
   discomfort was fair? The only publishable version is one where the
   other person's feelings are respected on the page.
4. The potential home, if it earns one: the two-worlds contrast — the same
   gesture that got flagged in corporate America is, in hospice, sometimes
   the whole visit. But that framing must not flatten the complainant into
   a foil. It may also belong to the departure story ("he said... where
   are the numbers" — same voice? Was this the same manager? The same
   conversation era?).

---

## 2026-08-31 — the girl who recruited me and nobody liked

> The girl who recruited me and nobody liked.

**Logged; character fragment for the origin story.** She was on the other
side of the interview Rob bluffed through — the person nobody liked was
the one who saw him. That's a door-opener in a book about people who open
doors, and it rhymes with the book's whole method (Rob sees the people
others don't; first, someone unlikeable saw him).

Questions before drafting:
1. Who was she — recruiter, hiring manager, district manager? Her role in
   the actual hiring: what did she see in a guy who didn't know type 1
   from type 2?
2. Why didn't people like her — and did Rob? (The honest version might be
   complicated: maybe she was hard to like AND right about him. Both can
   be true on the page.)
3. What happened to her — still in the industry? Still in touch? Would
   she get a name (with her blessing) or a changed one?
4. Home: the interview beat in "Type One, Type Two" (one paragraph), or a
   short portrait like Diabetes Dave if the material runs deeper. Rob's
   call once the questions are answered.

**Continuation, arrived incomplete:** "I would learn" — the sentence cut
off. Rob: finish it. ("I would learn…" what — why nobody liked her? what
she'd done for me? what she knew?) The unfinished sentence is likely the
portrait's hinge.

---

## 2026-08-31 — Lou and the reps (SACRED LANE)

> My dad would see the reps at his doctors' offices, and they loved my dad.

**Anchored, and sacred:** Lou, in the waiting rooms of this book's world —
the same offices, the same species of visitor as his son — and the reps
loved him. No quoted words for Lou, per the standing rule; this fragment
is a fact and a picture, which is exactly what can be placed.

Why it matters to THIS book: it's the full circle nobody could invent —
while Rob carried a bag into offices, his father was on the patient side
of the same rooms, being the man everyone loved. It quietly answers where
the salesman came from: Rob learned rooms from Lou.

Questions (Rob only):
1. When — during your Novo years? Was Lou seeing doctors in your world's
   era (2021–2024), and did any rep ever realize whose father he was?
2. One moment you witnessed or were told about — Lou with a rep, a
   receptionist, a waiting room. The picture, not dialogue.
3. Home: the Lou paragraph in "Number One in the Nation" (the doorway
   could open one inch wider: the reps loved him before the ballroom ever
   heard your name), or "Type One, Type Two" (where the salesman came
   from), or held for Book One. Your call — Lou's beats get placed by
   you, not by drafts.

---

## 2026-08-31 — the Jersey Shore DJ

> DJ from Jersey Shore DJs.

**Reading:** a DJ from Jersey Shore — most likely DJ Pauly D — DJ'd at
the meeting/party (the NSM? the Universal Studios night?). Confirm: was
it Pauly D, and which event? Naming a public figure performing a paid gig
is fine (same rule as Brady — factual, no invented words).

**Why it earns a line:** for the kid from New Jersey, a Jersey Shore DJ
at the top-of-the-list party is the counted ledger winking at him — the
scene texture for the keynote/Universal material (music too loud for nine
in the morning now has a name). Placement with the Brady/Universal scene
once year and city are pinned.

---

## 2026-08-31 — Universal was 2023

> They closed off Universal for us, 2023.

**Anchored: the Universal Studios night was 2023** — a national-meeting
event (Universal = Orlando) during the climb, the year BEFORE the Q3 2024
#1. Cascade:
- Number One chapter fixed: Universal is no longer framed as the #1
  reward — it's "the year before, at the national meeting."
- 2023 becomes the leading candidate for the Brady NSM (he retired Feb
  2023; an Orlando NSM later that year fits everything). Confirm: was
  Brady the 2023 Orlando meeting — Universal, Pauly D, Brady, one trip?

---

## 2026-08-31 — Dancing with the Stars, 2022

> Dancing with the Stars does a show, 2022.

**Anchored: at the 2022 national meeting, Dancing with the Stars
performed.** The meetings now form their own timeline of the boom:

- **2022** — DWTS does a show (Rob's first NSM, ~6 months into the job)
- **2023** — Universal Studios closed for the sales force; Brady keynote
  (candidate year); Pauly D (candidate)
- **2024** — the #1 quarter (Q3); meeting entertainment unknown

**The story shape this reveals:** each year's meeting got bigger as the
drug got more famous — the craze measured in corporate entertainment. The
rookie who didn't know type 1 from type 2 in August 2021 was watching
network television perform for his division by his first spring. The
keynote chapter (or a merged NSM chapter) can run this escalation as its
spine: DWTS → Brady → an empty theme park — and then the quarter that
actually mattered arrived as a number on a dashboard, two weeks late.

Confirm: which acts go with which years (memory drifts on this exact kind
of detail — worth a beat of care); what 2024's meeting looked like; and
whether the escalation read as glory or excess to you at the time (both,
probably — say it your way).

---

## 2026-08-31 — "Ozempic was worth more than Denmark"

> Ozempic was worth more than Denmark.

Already anchored in the craze chapter (company market value vs. Denmark's
GDP, Sept 2023, panel-verified with the "on paper" hedge). Logged here for
**Rob's cadence**: his compressed version — the drug, not the company — is
how a rep would say it out loud. Option for the chapter: keep the precise
version in narration, let his compression be the spoken/remembered line
("Somebody at a meeting put it the way we all repeated it: Ozempic was
worth more than Denmark."). If used as speech, it needs a true source —
did people actually say this in the field/at the NSM? Confirm.

---

## 2026-08-31 — my customers, the lawsuits, and the silence order

> Customers of mine who were selling the compound were getting lawsuits,
> and we were told not to talk to them at all.

**Anchored (and it completes the Pace scene's aftermath):**
1. Offices in Rob's own territory — his customers — were selling
   compounded semaglutide themselves.
2. The lawsuits landed on them (consistent with Novo's public litigation
   wave against clinics/med spas/compounders, 2023–24, heavily in
   Florida — public record).
3. The field directive: **do not talk to them. At all.** Rob's company
   suing Rob's customers, and the rep ordered silent — relationships he'd
   built ending by instruction, not conversation.

**Placed in the Wild West chapter** as the anchored middle: rival rep
(supply) → customers turning sellers + lawsuits + the wall (the war comes
to his own call list) → cookout (demand). Legal handling: no office
identifiable, the litigation described only at public-record level, the
directive described generically ("we were told") per the guardrails —
internal instructions as lived texture, never documents.

**Still needed:** roughly when the order came; whether any of those
customers ever reached out anyway (a voicemail he couldn't return is a
scene); how it felt driving past an office he was forbidden to enter.

---

## 2026-08-31 — "Novo Nordisk," and the nervousness

> I sat in a cafeteria or a waiting room, and reps said, oh, where do you
> work? I said, Novo Nordisk. And you could tell they got really nervous —
> because he was illegally selling the compound as a rep for a pharmacy.

**Anchored — the Pace scene's electric beat (or a second encounter):** the
exchange ran both ways. They asked where Rob worked; he said the company's
name; the nervousness was visible — because the name was, by then, a
plaintiff's name, and the compound rep (working for a pharmacy) knew which
side of the lawsuits he stood on.

Memory notes, honestly held: setting uncertain in Rob's own telling
("a cafeteria or a waiting room" vs. the earlier "in the office" at the
lunch) — could be the same encounter or a second one. The chapter keeps
one merged scene for now; Rob confirms whether it was one meeting or two,
and where. "Illegally selling" stays his lived read; the narration keeps
the legal nuance.

---

## 2026-08-31 — the doctor who injected himself at the lunch

> I was with my boss at a lunch, and a doctor says, give me one of those
> pens that he signed for. And he lifts his shirt up, and he injects
> himself.

**Anchored — the craze inside the prescriber class.** A doctor, at a rep
lunch, Rob's boss present, demanding a sample pen he'd signed for — then
lifting his shirt and injecting himself at the table. Legally received
(signed for), self-administered on the spot; not the rep's violation, and
nothing the rep could do — which is the beat's power: the hunger had
credentials, and the paperwork was in order.

Placed in "We Can't Sample Here Anymore" as the escalation before the
skinnier-staff office (it wasn't only staff). Composite rules apply: no
identifiable doctor/office; different office than the cutoff one unless
Rob says otherwise. Still needed: roughly when; what the boss said (or
didn't) in the car after — that car ride is a scene if it happened.
