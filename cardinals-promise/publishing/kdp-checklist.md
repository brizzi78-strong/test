...
|---|---|---|
| Manuscript (interior) | `kdp-interior.pdf` | **260 pages**, 8.5 × 11 in, no bleed, fonts embedded, folios + TOC match. **Re-exported July 2026** from the current master (rendered the 261-page book, then removed page 1 — the cover — leaving the 260 content pages KDP wants). Its 260-page spine matches the cover exactly, so the two files agree at upload. The full **cover + manuscript reading/proof copy** (261pp, cover on page 1) is `../book/cardinals-toolkit-book.pdf`. Now includes a Cardinal Moment in every chapter (29 total) and the Nick Adams, PTA quote in Chapter 6. Interior folios run 2→261 (the cover is sheet 1 and is unnumbered), so every folio matches the Contents by construction (Chapter 1 = 37, Chapter 6 = 77). |
| Book cover | **`kdp-cover-wrap-print.pdf`** (upload this) | One-piece wrap, **exactly 17.8602 × 11.25 in** with 0.125" bleed; **spine 0.6102"** (260 pages × 0.002347", **premium color** paper — recompute if ink choice changes); 2" × 1.2" white barcode zone, lower-right of back cover, left blank for KDP. Flattened single page at 300 DPI (5358 × 3375 px) so KDP reads the size exactly. The editable vector version is `kdp-cover-wrap.pdf` (Chromium renders it a few thousandths narrow; prefer the flattened `-print` file for upload). |
...
- [x] **Standard-spec cover built + flattened (July 2026)** — the wrap's front panel now carries the adopted crossout cream design (red cardinal, *IT'S ~~ALL~~ YOUR FAULT* with "Not" above, blue "From the Author of The Cardinal's Promise"), matching the standalone front. Two upload-ready files exist: `kdp-cover-wrap-print.pdf` (full wrap, exactly 17.8602 × 11.25 in, flattened) and `front-cover-print.pdf/.jpeg` (front-only, 8.75 × 11.25 in) for the Cover Creator path. Both are single-page, 300 DPI, dimension-exact.
      - ✅ **Interior and cover now agree (July 2026):** interior is **260 pages**, spine **0.6102"**, wrap **17.8602 × 11.25 in** — all recomputed from the current master. The earlier "too big" error came from a stale interior/cover mismatch; that's resolved, so the two files agree at upload.
