...
|---|---|---|
| Manuscript (interior) | `kdp-interior.pdf` | **259 pages**, 8.5 × 11 in, no bleed, fonts embedded, folios + TOC match. **Re-exported July 2026** from the current master (rendered the 260-page book, then removed page 1 — the cover — leaving the 259 content pages KDP wants). Its 259-page spine matches the cover exactly, so the two files agree at upload. The full **cover + manuscript reading/proof copy** (260pp, cover on page 1) is `../book/cardinals-toolkit-book.pdf`. Now includes a Cardinal Moment in every chapter (29 total) and the Nick Adams, PTA quote in Chapter 6. Interior folios run 2→260 (the cover is sheet 1 and is unnumbered), so every folio matches the Contents by construction (Chapter 1 = 36, Chapter 6 = 76). |
| Book cover | **`kdp-cover-wrap-print.pdf`** (upload this) | One-piece wrap, **exactly 17.8579 × 11.25 in** with 0.125" bleed; **spine 0.6079"** (259 pages × 0.002347", **premium color** paper — recompute if ink choice changes); 2" × 1.2" white barcode zone, lower-right of back cover, left blank for KDP. Flattened single page at 300 DPI (5357 × 3375 px) so KDP reads the size exactly. The editable vector version is `kdp-cover-wrap.pdf` (Chromium renders it a few thousandths narrow; prefer the flattened `-print` file for upload). |
