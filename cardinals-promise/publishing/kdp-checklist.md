...
|---|---|---|
| Manuscript (interior) | `kdp-interior.pdf` | **226 pages**, 8.5 × 11 in, no bleed, fonts embedded, folios + TOC + index match. **Re-exported July 2026** from the current master (rendered the 227-page book, then removed page 1 — the cover — leaving the 226 content pages KDP wants). Its 226-page spine matches the cover exactly, so the two files agree at upload. The full **cover + manuscript reading/proof copy** (227pp, cover on page 1) is `../book/cardinals-toolkit-book.pdf`. Note: interior folios run 2→227 (the cover is sheet 1 and is unnumbered), so every folio matches the Contents by construction (Chapter 1 = 36). |
